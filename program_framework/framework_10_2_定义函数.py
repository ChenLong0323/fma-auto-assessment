...
def draw_txt(image,width_det_pose,width_draw_sit,width_draw_shoulder,width_draw_hip,fps,trigger,count,move_time_l,min_list_l,move_time_r,min_list_r):
...
def main():
    ...
    thread.start()
    thread2 = threading.Thread(target=process_frame,
                               args=(k4a, model, mp_hands, frame_queue,
                                     width_det_pose, width_det_hand, running_flag))
    thread2.start()
    ...
    try:
        while True:
            if not frame_queue.empty():
                ...
                # 绘制文字
                image = draw_txt(image,width_det_pose,width_draw_sit,width_draw_shoulder,width_draw_hip,fps,trigger,count,move_time_l,min_list_l,move_time_r,min_list_r)
                # 绘制box，绘制四个点
                print(fps)
