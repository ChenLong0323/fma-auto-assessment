...
def main():
    ...
    trigger = deque(maxlen=trigger_len)
        ...
        for result in results:
            ...
            if len(keypoints) > max(indices):
                ...
                # Flag = 1 , 左手运动检测
                    ...
                    # 绘制左侧目标line
                    cv2.line(frame, (640, 310), (640 + 2 * shoulder_w, 310), (0, 255, 255), 2)

                    # 判断左腕和左肘的y坐标都超过左肩
                    if ext[1] < ext[9] and ext[13] < ext[9]:
                        trigger.append(1)
                        if sum(trigger) > 60:
                            flag = 3
                            # 触发flag后，清空trigger
                ...
                # Flag = 2 , 右手运动检测
                    ...
                    cv2.putText(frame, 'score:0', (600, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
                    # 绘制右侧目标line
                    cv2.line(frame, (640, 295), (640 - 2 * shoulder_w, 295), (0, 255, 255), 2)
                    # 判断右腕和右肘的y坐标都超过右肩
                    if ext[3] < ext[11] and ext[15] < ext[11]:
                        trigger.append(1)
                        if sum(trigger) > 60:
                            flag = 3
                            # 触发flag后，清空trigger
