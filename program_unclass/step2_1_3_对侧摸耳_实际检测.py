            ...
            if len(keypoints) > max(indices):
                ...
                if flag == 0:
                    ...
                    cv2.rectangle(frame,
                                  ...
                                  (0, 255, 255), 2)
                    # 判断左手、右髋
                    if (abs((ext[0] - ext[6] - win_w)) < win_w) and (abs((ext[1] - ext[7])) < win_h):
                        flag = 1
                    # 判断右手、左髋
                    elif (abs((ext[2] - ext[4] + win_w)) < win_w) and (abs((ext[3] - ext[5])) < win_h):
                        flag = 2
                ...
                # Flag = 1 , 左手运动检测
                    ...
                    # 判断左腕到左耳的距离
                    if (abs((ext[0] - ext[8])) < win_w) and (abs((ext[1] - ext[9])) < win_h):
                        flag = 3
                ...
                # Flag = 2 , 右手运动检测
                    ...
                    # 判断右腕到右耳的距离
                    if (abs((ext[2] - ext[10])) < win_w) and (abs((ext[3] - ext[11])) < win_h):
                        flag = 3
                else:
