...
def main():
    ...
    while True:
        ...
        frame_d = frame_d.reshape(720, 1280, 1)
        frame_i = capture.transformed_ir
        frame_i = frame_i.reshape(720, 1280, 1)
        # 把frame_d的shape 从720,1080 变成3维

        # 调用yolo检测
        results = model(frame, stream=True, conf=0.7, verbose=False)
        result_d = model(frame_d, stream=True, conf=0.7, verbose=False)
        result_i = model(frame_i, stream=True, conf=0.7, verbose=False)
