...
def process_frame(k4a, model, mp_hands, frame_queue, width_det_pose, width_det_hand, running_flag):
...
def draw_txt(image, frame_queue2, width_det_pose, width_draw_sit, width_draw_shoulder, width_draw_hip, fps, trigger, count, move_time_l, min_list_l,
             move_time_r,
             min_list_r):
    try:
        ...
        frame_queue2.put((image), timeout=0.1)
    except queue.Full:
        # 在队列满时忽略，以避免阻塞
        print("full_draw_txt")
    except Exception as e:
        print(f"Error in thread draw_txt: {e}")
...
def main():
    ...
    thread = threading.Thread(target=process_frame,
                              args=(k4a, model, mp_hands, frame_queue,
                                    width_det_pose, width_det_hand, running_flag))
    thread.start()
    thread2 = threading.Thread(target=draw_txt,
                               args=(image, frame_queue2, width_det_pose, width_draw_sit, width_draw_shoulder, width_draw_hip, fps, trigger, count,
                                     move_time_l,
                                     min_list_l,
                                     move_time_r,
                                     min_list_r))
