...
def process_frame(k4a, model, mp_hands, frame_queue, running_flag,
    ...
    while running_flag[0]:
        try:
            ...
            if len(keypoints) > max(indices) and None not in x_coordinates and None not in y_coordinates:
                ...
                # 右侧检测
                elif dict_hand["flag_handedness"] == 2:
            ...
            frame_queue.put((image, dict_pass, dict_hand, dict_result, trigger, count_sides, count), timeout=0.1)
        except queue.Full:
            # 在队列满时忽略，以避免阻塞
            print("full")
            continue
...
def main():
    # 本字典只用读取，不用修改
    dict_read = {
        "width_det_pose": 350,  # 用于姿势检测的图像宽度，同时也是实际画布的绘制宽度
        "width_det_hand": 350,  # 用于手部检测的图像宽度
    ...
    dict_hand = {
        "flag_handedness": 0,  # 0 没检测，1检测左手，2检测右手
    ...
    trigger = deque(maxlen=60)  # 初始化 trigger，长度为100，初始空白
    count_sides = 0  # 计算左右手循环次数
    count = 0  # 计算完成项目次数
    ...
    thread = threading.Thread(target=process_frame, args=(k4a, model, mp_hands, frame_queue, running_flag,
                                                          dict_read, dict_pass, dict_hand, dict_result,
                                                          indices, trigger, count_sides, count
                                                          ))
    thread.start()
    ...
    try:
        while True:
            if not frame_queue.empty():
                image, dict_pass, dict_hand, dict_result, trigger, count_sides, count = frame_queue.get()
