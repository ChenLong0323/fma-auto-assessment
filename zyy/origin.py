...
try:
    ...
    while True:
        ...
        if len(x_data) > window_size:
            ...
            #-------------------------------------------------------------------
            # 保存数据到 CSV 文件
            with open('ljztest_8.csv', 'a', newline='') as csv_file:  #修改文件名 whs_Walking_1.csv 名字_实验任务
                csv_writer = csv.writer(csv_file)
                csv_writer.writerow(data_save)
            # -------------------------------------------------------------------
            # 检查阈值条件:任意一个值大于T
            if any(y > T for y in data_save):
                # 触发蜂鸣器
                winsound.Beep(1000, 100)  # 指定蜂鸣器频率和持续时间，单腿阈值
